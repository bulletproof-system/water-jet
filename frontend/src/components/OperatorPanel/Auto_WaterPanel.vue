<template>
	<div class="d-flex flex-column ">
		<v-btn class="ma-2 bg-primary" size="large" :disabled="disabled" 
			@click="nodeInfo.state === NodeState.Auto_Water ? rosStore.cancel() : ctrl.autoWater()"
		> 
			{{ nodeInfo.state === NodeState.Auto_Water ? '停止' : '开始巡检' }}
		</v-btn>
	</div>
</template>
...
const disabled = computed(() => 
	!(nodeInfo.value.state === NodeState.Wait || nodeInfo.value.state === NodeState.Auto_Water) || 
