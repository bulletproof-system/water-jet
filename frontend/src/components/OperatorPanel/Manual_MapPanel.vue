<template>
	<div class="d-flex flex-column ">
		<v-btn class="ma-2 bg-primary" size="large" :disabled="disabled" 
			@click="nodeInfo.state === NodeState.Manual_Init_Map ? rosStore.cancel() : ctrl.manualInitMap()"
		> 
			{{ nodeInfo.state === NodeState.Manual_Init_Map ? '停止建图' : '开始建图' }}
		</v-btn>
	</div>
</template>
...
const disabled = computed(() => 
	!(nodeInfo.value.state === NodeState.Wait || nodeInfo.value.state === NodeState.Manual_Init_Map) || 
